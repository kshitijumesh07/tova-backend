...
function createBooking(order_id, ride_id, user_id, seats_available) {
  const db = load();
  db.bookings.push({ order_id, ride_id, user_id, status: "CREATED", seats_available, created_at: new Date().toISOString() });
  save(db);
...
function confirmBooking(order_id, user_id) {
  const db = load();
  const booking = db.bookings.find((b) => b.order_id === order_id && b.user_id === user_id);

  if (!booking) {
    console.warn("BOOKING NOT FOUND:", order_id, user_id);
...
function failBooking(order_id, user_id) {
  const db = load();
  const booking = db.bookings.find((b) => b.order_id === order_id && b.user_id === user_id);
  if (booking) {
    booking.status = "FAILED";
    save(db);
    console.log("BOOKING FAILED:", order_id, user_id);
  }
}

function updateRideStatus(order_id, user_id, status) {
  const db = load();
  const booking = db.bookings.find((b) => b.order_id === order_id && b.user_id === user_id);
  if (booking) {
